...
function mediaUrl(value: string) { return value.startsWith("/objects/") ? `/api/storage${value}` : value; }
...
function ContentEditor() {
  ...
  async function uploadImage(file: File) { const r = await request("/api/storage/uploads/request-url", { method: "POST", body: JSON.stringify({ name: file.name, size: file.size, contentType: file.type }) }); const result = await r.json(); if (!r.ok) { setMessage(result.error ?? "Зураг upload хийхэд алдаа гарлаа."); return; } const put = await fetch(result.uploadURL, { method: "PUT", body: file, headers: { "Content-Type": file.type } }); if (put.ok) { setData((old) => ({ ...old, [selected]: { ...(old[selected] ?? {}), image_url: result.objectPath } })); setMessage("Зураг бэлэн боллоо. Хадгална уу."); } }
